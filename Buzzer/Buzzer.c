...
void buzzer_tone(Buzzer_cfg_t* cfg, uint32_t freq_hz, uint8_t volume_percent)
{
    ...
    // ARR = (tick_freq / freq) - 1  gives the correct PWM period
    ...
    // Cap at 50% duty for a clean square-wave tone; volume scales within that half
...
{
    // C5 -> E5 -> G5 ascending sting
    static const uint32_t notes[] = { NOTE_C5, NOTE_E5, NOTE_G5 };
    for (int i = 0; i < 3; i++) {
        buzzer_tone(cfg, notes[i], 100);
        HAL_Delay(100);
    }
    buzzer_off(cfg);
}
...
{
    // G5 -> C5 -> C4 descending drop
    static const uint32_t notes[] = { NOTE_G5, NOTE_C5, NOTE_C4 };
    for (int i = 0; i < 3; i++) {
        buzzer_tone(cfg, notes[i], 100);
        HAL_Delay(150);
    }
